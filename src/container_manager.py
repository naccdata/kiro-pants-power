...
class ContainerManager:
    ...
    def __init__(
        self, executor: CommandExecutor | None = None, workspace_folder: Path | None = None
    ):
    ...
    def ensure_running(self) -> bool:
        ...
        try:
            ...
            if result.success:
                ...
                error_msg = (
                    f"Container start failed: devcontainer up exited with "
            ...
            raise ContainerError(
                f"Failed to start container: {e!s}\n\n"
                "Please check that Docker is running and the devcontainer CLI "
    ...
    def start(self) -> CommandResult:
        ...
        try:
            ...
            if not result.success:
                error_msg = (
                    f"Container start failed: devcontainer up exited with code "
                    f"{result.exit_code}\n\n"
                    f"Output:\n{result.output}\n\n"
                    "Troubleshooting:\n"
                    "1. Check if Docker Desktop is running\n"
            ...
            raise ContainerError(f"Failed to start container: {e!s}") from e
    ...
    def stop(self) -> CommandResult:
        ...
        command = (
            f"devcontainer exec --workspace-folder {self.workspace_folder} "
            "hostname | xargs docker rm -f"
        )
        ...
        try:
            ...
            raise ContainerError(f"Failed to stop container: {e!s}") from e
    ...
    def rebuild(self) -> CommandResult:
        ...
        try:
            ...
            if not build_result.success:
                error_msg = (
                    f"Container build failed: devcontainer build exited with code "
            ...
            if not up_result.success:
                error_msg = (
                    f"Container start after build failed: devcontainer up exited "
                    f"with code {up_result.exit_code}\n\n"
                    f"Output:\n{up_result.output}"
                )
                raise ContainerError(error_msg)
            ...
            raise ContainerError(f"Failed to rebuild container: {e!s}") from e
    ...
    def exec(self, command: str) -> CommandResult:
        ...
        try:
            ...
            raise ContainerError(f"Failed to execute command in container: {e!s}") from e
